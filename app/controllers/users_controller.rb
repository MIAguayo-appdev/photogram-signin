class UsersController < ApplicationController
  ...
  def authenticate
    ...
    if user == nil 
     redirect_to("/user_sign_in", { :alert => "username nonexistent" })
    else
      #if there is a record, check to see if password matches
      #if so, set the cookie 
      #redirect to homepage
      if user.authenticate(pw) #this returns the record, any non nil counts as true
        session.store(:user_id, user.id)
        redirect_to("/", { :notice => "Welcome back " + user.username + "!" })
      else
        #if password does not match, redirect back to sign in form
  ...
  def create
    ...
    if save_status == true
      session.store(:user_id, user.id) #to sign in 
